...
data$N_case_r7 <- stats::filter(data$N_case,rep(1/7,7),sides=1)
...
p <- ggplot(plotdata, aes(x=date)) +
    geom_line(aes(y = N_case_r7), color = "red",  size = 1) +
    scale_x_date(date_breaks = "14 day", date_labels = "%d/%m") +
    xlab(" ") +
    labs(title = "Daily number of new Covid-19 cases (7-day rolling average)",
         caption = paste0("Source: Public Health Agency of Sweden. Updated: ", lastdate,"."),
         x = " ",
         y = "Number of Covid-19 cases") + theme_linedraw() + theme(panel.border = element_blank(),
...
ggsave(filename = file.path("docs", "dailycovidcase.png"), plot = p,
